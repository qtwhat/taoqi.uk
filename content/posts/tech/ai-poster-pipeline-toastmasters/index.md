...
交给 AI 自动化？判断这部分得我自己来；剩下的重复劳动我既不擅长也不想做，正好都给 AI。

动机于是很清楚：把劳动尽量交出去，每周只留下需要我拍板的部分，最好到最后只需要给一个 theme。

我就这么半开玩笑地跟 Claude Code（下面简称 CC）许了个愿。
...
用 Agent 的一条大原则：每个要重复的纯机械动作，都值得花一次功夫，让它以后自动跑。

## 自动化的最后一步：调用 Codex CLI 直接画图

做到这里，整条流程只剩最后一个手动动作：去 ChatGPT 网页里粘贴 prompt，生成背景图。
...
这周就是现成的例子：第 1024 期，主题 The Best... I Want to Recommend。我给 AI 的输入就是这一句话，文章开头那张海报便是这么来的。
...
如果你也在做 VPPR，或者想自动化手头的任何流程，我会先建议你把事情拆开看：哪些是必须你拍板的，比如风格、信息主次、彩蛋；哪些只是体力活，比如画图、对齐、导出。后面这一类最适合交给工具，前面那一类别急着丢给 AI，不然海报会很快变得正确但没意思。
