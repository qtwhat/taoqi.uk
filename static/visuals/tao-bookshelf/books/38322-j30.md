...
3 Definitions, symbols and abbreviations [6](#definitions-symbols-and-abbreviations)
...
4.2.1.2 UM RLC entity [10](#um-rlc-entity)

4.2.1.2.1 General [10](#general-2)
...
4.3.1 Services provided to upper layers [12](#services-provided-to-upper-layers)

4.3.2 Services expected from lower layers [12](#services-expected-from-lower-layers)
...
5.2.2.2.2 Actions when an UMD PDU is received from lower layer [14](#actions-when-an-umd-pdu-is-received-from-lower-layer)

5.2.2.2.3 Actions when an UMD PDU is placed in the reception buffer [15](#actions-when-an-umd-pdu-is-placed-in-the-reception-buffer)

5.2.2.2.4 Actions when t-Reassembly expires [15](#actions-when-t-reassembly-expires)
...
5.2.3.2.2 Actions when an AMD PDU is received from lower layer [17](#actions-when-an-amd-pdu-is-received-from-lower-layer)

5.2.3.2.3 Actions when an AMD PDU is placed in the reception buffer [17](#actions-when-an-amd-pdu-is-placed-in-the-reception-buffer)

5.2.3.2.4 Actions when *t-Reassembly* expires [18](#actions-when-t-reassembly-expires-1)

5.2.3.2.5 Actions when *t-RxDiscard* expires [18](#actions-when-t-rxdiscard-expires)

5.3 ARQ procedures [19](#arq-procedures)

5.3.1 General [19](#general-10)

5.3.2 Retransmission [19](#retransmission)

5.3.3 Polling [20](#polling)

5.3.3.1 General [20](#general-11)

5.3.3.2 Transmission of a AMD PDU [20](#transmission-of-a-amd-pdu)

5.3.3.3 Reception of a STATUS report [21](#reception-of-a-status-report)

5.3.3.4 Expiry of *t-PollRetransmit* [21](#expiry-of-t-pollretransmit)

5.3.4 Status reporting [21](#status-reporting)

5.4 SDU discard procedures [23](#sdu-discard-procedures)

5.5 Data volume calculation [23](#data-volume-calculation)

5.6 Handling of unknown, unforeseen and erroneous protocol data [23](#handling-of-unknown-unforeseen-and-erroneous-protocol-data)

5.6.1 Reception of PDU with reserved or invalid values [23](#reception-of-pdu-with-reserved-or-invalid-values)

6 Protocol data units, formats and parameters [24](#protocol-data-units-formats-and-parameters)

6.1 Protocol data units [24](#protocol-data-units)

6.1.1 General [24](#general-12)

6.1.2 RLC data PDU [24](#rlc-data-pdu)

6.1.3 RLC control PDU [24](#rlc-control-pdu)

6.2 Formats and parameters [24](#formats-and-parameters)

6.2.1 General [24](#general-13)

6.2.2 Formats [24](#formats)

6.2.2.1 General [24](#general-14)

6.2.2.2 TMD PDU [24](#tmd-pdu)

6.2.2.3 UMD PDU [25](#umd-pdu)

6.2.2.4 AMD PDU [26](#amd-pdu)

6.2.2.5 STATUS PDU [27](#status-pdu)

6.2.3 Parameters [28](#parameters)

6.2.3.1 General [28](#general-15)

6.2.3.2 Data field [29](#data-field)

6.2.3.3 Sequence Number (SN) field [29](#sequence-number-sn-field)

6.2.3.4 Segmentation Info (SI) field [29](#segmentation-info-si-field)

6.2.3.5 Segment Offset (SO) field [29](#segment-offset-so-field)

6.2.3.6 Data/Control (D/C) field [29](#datacontrol-dc-field)

6.2.3.7 Polling bit (P) field [30](#polling-bit-p-field)

6.2.3.8 Reserved (R) field [30](#reserved-r-field)

6.2.3.9 Control PDU Type (CPT) field [30](#control-pdu-type-cpt-field)

6.2.3.10 Acknowledgement SN (ACK_SN) field [30](#acknowledgement-sn-ack_sn-field)

6.2.3.11 Extension bit 1 (E1) field [30](#extension-bit-1-e1-field)

6.2.3.12 Negative Acknowledgement SN (NACK_SN) field [31](#negative-acknowledgement-sn-nack_sn-field)

6.2.3.13 Extension bit 2 (E2) field [31](#extension-bit-2-e2-field)

6.2.3.14 SO start (SOstart) field [31](#so-start-sostart-field)

6.2.3.15 SO end (SOend) field [31](#so-end-soend-field)

6.2.3.16 Extension bit 3 (E3) field [31](#extension-bit-3-e3-field)

6.2.3.17 NACK range field [32](#nack-range-field)

7 Variables, constants and timers [32](#variables-constants-and-timers)

7.1 State variables [32](#state-variables)

7.2 Constants [34](#constants)

7.3 Timers [34](#timers)

7.4 Configurable parameters [34](#configurable-parameters)

Annex A (informative): Change history [35](#annex-a-informative-change-history)
...
\- if RX_Next\_ Discard_Trigger = RX_Next + 1 and there is no missing byte segment of the SDU associated with SN = RX_Next before the last byte of all received segments of this SDU; or
